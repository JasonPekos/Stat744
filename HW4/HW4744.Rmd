...
## BMB: could read from a data file ???

day = c(1:length(deaths))  ## don't need c(); seq_along(deaths) slightly better
...
**BMB: confidence interval on smooth goes below zero.  Could limit scale with `oob=scales::squish`, or
better to use a smoother with a log link (e.g. `geom_smooth(method="mgcv", method.args=list(family=quasipoisson))`).
Legend seems a little weird (I think I would put this in a footnote/subtitle/caption rather than as a legend)**
...
**BMB: I still have Opinions about these priors, but not relevant here**
...
chains = tibble(read.csv("ModelOut.csv")) ## BMB: could just use read_csv(), would get as a tibble immediately
chains = rename(chains, sigma = σ, beta = β, gamma = γ, mu = μ) #dumb, from julia code
## BMB: can you get away with keeping the symbols through the R code? (Might work, although much more fragile/may break in some contexts)
...
**BMB: this is a little tricky, but what are the units on the y-axis? I don't think beta and mu match, unless you've scaled beta appropriately (N=1).
Depending on the intended audience, you might want to add natural-language descriptions of the parameters ("death rate", "contact rate")**
...
We can plot every posterior distribution separately also:
...
## BMB: there's probably a tidier way to do the manipulations above ...

gg2 <- ggplot(mdatDeath, aes(x = time,y =value)) +
...
Notice how the inferred infection category (for which we lack any data) has a larger range of uncertainty in the distribution of possible fits compared to the deaths time series for which we have data. \
...
**BMB: we can fit a unimodal spline (although this seems to oversmooth ...)**

```{r}
uu <- with(Eyam, unireg(day, deaths, sigmasq=1))
upred <- tibble(day=Eyam$day, deaths = predict(uu, newdata=Eyam$day))
gg2 + geom_line(data=upred, aes(x=day, y=deaths), colour="purple", lwd=2)
```

**BMB: mark 2.2/3. There's a lot of effort here, but I think more of the emphasis was on the modeling rather than on the graphical display. How did you choose colours etc.? Do we need to see the whole distributions? Would violin plots be more compact? Or maybe we only need 50% and 95% intervals?**
